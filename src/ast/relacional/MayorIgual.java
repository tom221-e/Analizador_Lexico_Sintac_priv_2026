...
public class MayorIgual extends OperacionBinaria {
    ...
    public String get_llvm_op_code() {
        ...
        if (this.tipo.matches("\\d+")) {
            return "8"; // Cambia este identificador por el número que use tu ALU para ">="
        }
        ...
        switch (this.tipo) {
            case "float":
            case "double":
                return "fcmp oge"; // 'oge' significa Ordered Greater or Equal
            case "i32":
            case "boolean":
            default:
                return "icmp sge"; // 'sge' significa Signed Greater or Equal
        }
    }

    /**
     * Genera la instrucción de comparación relacional escalar para "Mayor o Igual que".
    ...
    protected String obtenerCodigoEscalar() {
        ...
        // Formato Enteros:  %ptro.X = icmp sge i32 %ptro.A, %ptro.B
        // Formato Decimales: %ptro.Y = fcmp oge double %ptro.C, %ptro.D
